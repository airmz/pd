  ...
  <div id="img-water-fall" ref="imgWaterFall">
    <div class="content-box" :style="{
      ...
      <div
        ...
        <!-- @mouseover="selectMouseImg(item, index)"
        @mouseout="dActiveElement.uuid !== '-1' ? handle('delete') : ''" -->
        <transition name="fade">
          <img
            class="img"
             v-lazy="item.value.url"
            :style="{
...
export default {
  ...
  methods: {
    ...
    selectImg (value, index) {
      ...
      //             this.selectIndexImg=this.content.background;
      //         });
    },
    selectMouseImg (value, index) {
      this.$emit('select-img', value);
      this.getWidgetJsonData().then((json) => {
                this.content = JSON.stringify(json);
                  this.selectIndexImg=this.content.background;
    ...
    mouseoutImg(item){
      if(this.selectIndexImg!=''){
         this.updatePageData({
          key: "backgroundImage",
          value: this.selectIndexImg,
        });
      }
      this.selectIndexImg="";
        // this.updatePageData({
