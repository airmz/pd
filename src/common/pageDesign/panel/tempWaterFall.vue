<template>
  <div id="temp-water-fall" ref="tempWaterFall">
...
export default {
  ...
  mounted () {
    this.parentWidth = this.$refs.tempWaterFall.clientWidth
    this.preLoadImg()
    this.$refs.scrollBar.onmousedown = this.tragScroll
    this.$refs.scrollBox.onmousedown = this.clickScroll
    this.mouseWheel(this.$refs.tempWaterFall, this.wheelChange)
  ...
  methods: {
    ...
    checkedImg (){
      var that=this
      setTimeout(function(){
        // console.log(that.checkedImgs)
        that.$emit('checked-img', that.checkedImgs)
      },0)
    },
    ...
    wheelChange (data) {
      ...
      if (top + barHeight >= boxHeight) {
        console.log('-----------------触底了-------------');
        this.$emit('scroll-img', this)
      }
